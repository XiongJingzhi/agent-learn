...
When a user request matches a skill's description, use the load_skill tool to get detailed instructions before proceeding."""
    ...
    def _create_agent(self):
        ...
        # 检测 GLM 模型并设置 provider
        # GLM 模型使用兼容 OpenAI 的 API
        model_provider = None
        is_glm_model = self.model_name.startswith("glm")
        if is_glm_model:
            model_provider = "openai"

        # Extended Thinking 配置（仅 Anthropic 模型支持）
        # GLM 等 OpenAI 兼容模型不支持 thinking 参数
        if self.enable_thinking and not is_glm_model:
